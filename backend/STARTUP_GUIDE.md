...
知识库与智能问答需要 Qdrant 和 SiliconFlow；不启用它们时，认证、工单等存量模块仍可运行。
...
- 新数据库：重新执行 `backend/sql/novaops_init.sql`
- 旧数据库：备份后执行 `backend/sql/refactor_rag_migration.sql`

为后端进程设置环境变量（不要写入前端 `.env.development`）：
...
启动后先用管理员登录，进入“知识库”上传文档。文档达到 `READY` 后，可从头像菜单进入“智能问答”。`GET /api/kb/documents/health` 可检查 Qdrant 是否可用。

生产环境必须启用 HTTPS，并在反向代理中关闭 SSE 响应缓冲、把读取超时设为大于后端 `NOVAOPS_AGENT_SSE_TIMEOUT_MS`。
