...
public class AgentService {
  ...
  private final AgentMapper mapper; private final KbRetrievalService retrievalService; private final ChatClient chatClient; private final CitationValidator validator; private final ObjectMapper objectMapper; private final ChatIntentClassifier intentClassifier=new ChatIntentClassifier(); private final long timeout; private final int topK; private final double minScore;
  ...
  public SseEmitter chat(CurrentSession session,ChatRequest request){
    ...
    if(intentClassifier.classify(request.getContent())==ChatIntentClassifier.Intent.GENERAL_CHAT){completeGeneralChat(emitter,conversation.getId(),intentClassifier.response(request.getContent()));return emitter;}
