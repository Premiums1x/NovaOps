...
const hashSeed = (day: string) => {
  let hash = 0
  for (let i = 0; i < day.length; i += 1) {
    hash = (hash * 31 + day.charCodeAt(i)) % 9973
  }
  return hash
}
...
export const buildDashboardMetrics = (
  ...
  const avgHandleHours = Math.round((12 + (hashSeed('avg') % 26) * 0.35) * 10) / 10
  const urgentRate = Math.round((8 + (hashSeed('urgent') % 11) * 0.7) * 10) / 10

  const categories = [
    { name: '故障工单', value: 32 + (hashSeed('fault') % 16) },
    { name: '需求工单', value: 24 + (hashSeed('feature') % 14) },
    { name: '巡检工单', value: 16 + (hashSeed('inspection') % 12) },
    { name: '安全工单', value: 10 + (hashSeed('security') % 8) },
  ]

  const durations = [
    { name: '网络类', hours: Math.round((8 + (hashSeed('net') % 10) * 0.8) * 10) / 10 },
    { name: '终端类', hours: Math.round((6 + (hashSeed('client') % 9) * 0.9) * 10) / 10 },
    { name: '系统类', hours: Math.round((9 + (hashSeed('system') % 10) * 0.9) * 10) / 10 },
    { name: '权限类', hours: Math.round((4 + (hashSeed('auth') % 8) * 0.7) * 10) / 10 },
    { name: '资产类', hours: Math.round((5 + (hashSeed('asset') % 9) * 0.8) * 10) / 10 },
  ]
